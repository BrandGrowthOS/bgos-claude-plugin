...
test('win32 spec: run-hidden.vbs plus a generated install-task.ps1 driven through the Task Scheduler API (never schtasks /SC ONLOGON)', () => {
  ...
  assert.match(script.content, /\$watcherScript = 'C:\Users\kc\\.bgos-agent\watcher\bin\hoai-watcher\.mjs'/)
  assert.match(script.content, /\$_\.Name -eq "node\.exe" -and \$_\.ProcessId -ne \$PID -and \$_\.CommandLine -like \("\*" \+ \$watcherScript \+ "\*"\)/)
  assert.ok(!/-match "hoai-watcher\.mjs"/.test(script.content), 'never a bare file-name match')
