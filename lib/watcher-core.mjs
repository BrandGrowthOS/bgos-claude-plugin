...
export async function observeMachine({
  ...
  return { state, configDir, pluginRoot, installMethod, agents, missingTargets, marketplace, clone }
}
 ...
 */
export async function refreshWatcherIfStale({ home, fs, now, manifest, pluginRoot, log = () => {} }) {
  if (!pluginRoot) return { needed: false, ok: true, message: 'watcher_bundle_source_unknown', fingerprint: null }
  ...
  try {
    const staged = await installWatcherBundle({ pluginRoot, home, fs, now, targetDir: joinDir(watcherHome(home), STAGING_DIR_NAME) })
...
export async function runReconcileJob(ctx, rpcId, job) {
  ...
  const deps = {
    ...
    git: async (args, cwd) => toCliResult(await exec('git', args, { cwd: cwd || observed.pluginRoot, timeoutMs: GIT_TIMEOUT_MS })),
  ...
  if (result.ok) {
    ...
    const refresh = await refreshWatcherIfStale({ home, fs, now, manifest, pluginRoot: currentRoot, log })
    if (refresh.needed) {
      ...
      else exitCode = selfRestartExitCode({ platform, home, spawnDetached: ctx.spawnDetached, log })
    }
  }
...
export async function runWatcher(deps = {}) {
  ...
  if (manifest?.claudeConfigDir && !String(env.CLAUDE_CONFIG_DIR ?? '').trim()) {
