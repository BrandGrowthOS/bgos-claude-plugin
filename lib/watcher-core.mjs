 ...
 */
/**
 * How large the watcher log may grow before it is rolled over to .1 and started again.
 *
 * There was no ceiling at all. The file is written by a daemon that runs forever and retries on a
 * 30 second timer, so a machine stuck in any repeating condition grew it without limit, on the user's
 * own disk. Five megabytes is far more history than this log has ever been read for.
 */
export const WATCHER_LOG_MAX_BYTES = 5 * 1024 * 1024

/** How often the credential wait repeats itself while it waits. */
export const CREDENTIAL_NOTICE_INTERVAL_MS = 60 * 60 * 1000

/** Roll the log over when it is too big. Best effort: a failure here must never lose a line. */
function rotateIfHuge(fs, path) {
  if (typeof fs.size !== 'function' || typeof fs.rename !== 'function') return
  try {
    const bytes = fs.size(path)
    if (typeof bytes !== 'number' || bytes < WATCHER_LOG_MAX_BYTES) return
    fs.rename(path, `${path}.1`)
  } catch {
    // Cannot stat, cannot rename, or something else owns the file. Keep appending: an oversized log
    // is a smaller problem than a lost line on the path that explains why the watcher is stuck.
  }
}
  ...
  return (message) => {
    ...
    try {
      if (typeof fs.appendFile === 'function') {
        // The read-then-write below is O(n) per line, so a long-running watcher spends more and more
        // time and memory on each entry, and a 30 second retry loop makes that a real cost rather
        // than a theoretical one. Append when the filesystem can; the fallback stays for injected
        // test filesystems that cannot.
        rotateIfHuge(fs, path)
...
export async function refreshWatcherIfStale({ home, fs, now, manifest, pluginRoot, exec = null, nodePath = null, log = () => {} }) {
  ...
  // nothing at all, forever. Surface it instead. The callers render it as a SKIPPED row, not a
  // failed one, and no longer let it rewrite the job's own message: the plugin update the user asked
  // for has already succeeded, and a red row under a green headline reads as a failed update.
  // The caller still refuses to self-restart on it (that requires needed && ok), so this changes
  // what is REPORTED, not what is done.
...
export async function runReconcileJob(ctx, rpcId, job) {
  ...
  if (plan.verdict === 'nothing_to_do' || steps.steps.length === 0) {
    ...
    // 'skipped' rather than 'failed', for the same reason as the other call site below: this row is
    // about the WATCHER's own bundle, not about the job the user asked for.
    if (refresh.needed) {
      steps.add({
        id: 'refresh_watcher',
        kind: 'refresh_watcher',
        state: refresh.ok ? 'ok' : 'skipped',
  ...
  if (result.ok) {
    ...
    if (refresh.needed) {
      // 'skipped', not 'failed', when the watcher could not refresh ITSELF. The plugin update the
      // user asked for has already happened; this row is about the watcher's own bundle. A red row
      // under a green headline reads as a failed update, which it is not.
      steps.add({
        id: 'refresh_watcher',
        kind: 'refresh_watcher',
        state: refresh.ok ? 'ok' : 'skipped',
      ...
      // And it must NEVER rewrite the job's own message when the job succeeded. That line turned a
      // successful update into one reporting a watcher-internal reason, on the one machine shape
      // where the plugin root cannot be resolved. The row above still carries the detail.
      if (!refresh.ok && !result.ok) message = refresh.message
      else if (refresh.ok) exitCode = refresh.exitCode
    }
  }
...
export async function runCreateAgentJob(ctx, rpcId, job) {
  ...
  steps.begin('preseed')
  try {
    // Pass rename when the injected fs has it, so this takes the write-then-rename branch. Without
    // it the watcher writes settings.json in place, and a `claude` reading at that moment sees a
    // truncated file and treats it as corrupt. The optional chaining keeps a test fs without rename
    // valid, which is why the branch is opt-in rather than required.
    preseedClaudeTrust({
      configDir,
      // The env this job resolved its config dir from, and the home it runs under: the config FILE
      // is $CLAUDE_CONFIG_DIR/.claude.json or $HOME/.claude.json, which is not the same answer as
      // the config DIR (claudeConfigFilePath). Passing them keeps the seed on the file the agents
      // launched from here will actually read.
      env,
      home,
      cwd: folder,
      fs: {
        readFile: fs.readFile,
        writeFile: (p, t) => fs.writeFile(p, t),
        ...(typeof fs.rename === 'function' ? { rename: (a, b) => fs.rename(a, b) } : {}),
        ...(typeof fs.remove === 'function' ? { remove: (a) => fs.remove(a) } : {}),
        // Without exists() a null read means "no file yet", and the seed would replace an owner
        // config it merely failed to read. Without chmod() the rename widens a 0600 file.
        ...(typeof fs.exists === 'function' ? { exists: (a) => fs.exists(a) } : {}),
        ...(typeof fs.chmod === 'function' ? { chmod: (a, m) => fs.chmod(a, m) } : {}),
        ...(typeof fs.modeOf === 'function' ? { modeOf: (a) => fs.modeOf(a) } : {}),
      },
    })
  ...
  // --no-install-cli: pairing puts the `hoai` command on PATH for the owner who
  // asked for it and is watching it happen. THIS caller is a background daemon
  // (KC's ruling, 2026-09-21). A daemon writing ~/.zshrc and ~/.bashrc with
  // nobody at a terminal to read the notes buys little, because hoai already
  // repairs its own launch, and costs trust: on a machine whose shell profiles
  // are generated (nix home-manager, a managed image) the write fails or is
  // reverted and the only record is in this daemon's scrubbed log. Ask or
  // announce, never silently replace.
  const pairArgs = [
    pairScript,
    pairCode,
    '--assistant-id',
...
export async function runWatcher(deps = {}) {
  ...
  // Said once, then at most hourly. This loop runs every 30 seconds and never gives up, so on a
  // machine that is never enrolled it wrote 2,880 identical lines a day into a log the logger
  // rewrites in full on every append. The information is worth having; repeating it 120 times an
  // hour is not.
  let lastCredentialNotice = 0
  while (!credentials) {
    const at = now()
    if (lastCredentialNotice === 0 || at - lastCredentialNotice >= CREDENTIAL_NOTICE_INTERVAL_MS) {
      lastCredentialNotice = at
