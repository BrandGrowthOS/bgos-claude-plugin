...
/**
 * WHAT IS ALREADY SITTING AT THE SHIM PATH, and whether it is ours.
 *
 * KC's rule, 2026-09-21, after review found two ways this call could change
 * something the owner never asked it to change: ASK OR ANNOUNCE, NEVER
 * SILENTLY REPLACE. Overwriting another install's shim is the same class of
 * defect as a first install failing silently. It reports success and changes
 * the meaning of a command the owner already had.
 *
 * Four states, told apart without a new effect (readlink answers null both for
 * "absent" and for "present but not a symlink", and exists() follows the link
 * so a BROKEN symlink reads as absent):
 *   readlink != null                 a symlink, and readlink is its target
 *   readlink == null && exists       something real that is NOT a symlink
 *   readlink == null && !exists      nothing there
 *
 * 'ours' means it already points exactly where we were going to point it, so
 * writing it changes nothing and re-writing it is free.
 * @returns {{ state: 'absent' | 'ours' | 'foreign-link' | 'foreign-file',
 *             pointsAt: string, targetExists: boolean }}
 */
export function inspectExistingShim(linkPath, wantedTarget, fx) {
  const link = String(linkPath ?? '')
  const wanted = String(wantedTarget ?? '')
  const current = fx.readlink(link)
  if (current != null) {
    const same = normalizeShimPath(current) === normalizeShimPath(wanted)
    return {
      state: same ? 'ours' : 'foreign-link',
      pointsAt: String(current),
      targetExists: fx.exists(String(current)),
    }
  }
  if (fx.exists(link)) return { state: 'foreign-file', pointsAt: link, targetExists: true }
  return { state: 'absent', pointsAt: '', targetExists: false }
}

/** Compare two shim paths: trailing separators and separator style only. */
function normalizeShimPath(path) {
 ...
 *   force?: boolean,
 * }} opts
 *   `force` is the deliberate override that re-points a shim belonging to
 *   another install. Without it such a shim is LEFT ALONE and reported.
 * @returns {{ ok: boolean, binDir: string, wrote: string[], notes: string[],
 *             onPath: boolean, profiles: string[],
 *             kept: Array<{ path: string, pointsAt: string, wanted: string,
 *                           reason: string }> }}
 *   `kept` is every shim we declined to overwrite. It is never a failure:
 *   declining is the correct outcome, and the caller PRINTS it, because the
 *   existing success line is guarded on `wrote.length > 0` and a silent
 *   decline would report success while writing nothing.
 */
export function installWrapper({
  ...
  const notes = []
  const profiles = []
  // Shims that were LEFT ALONE because they belong to another install. Never
  // a failure: declining to overwrite is the correct outcome, and the caller
  // prints it so the owner can decide. See inspectExistingShim.
  const kept = []
  const localAppData = String(env?.LOCALAPPDATA ?? '').trim() || null
  const plan = planWrapperInstall({ platform, home, localAppData, pluginRoot })
  if (!plan.ok) return { ok: false, binDir: '', wrote, notes: [plan.reason], onPath: false, profiles, kept }

  fx.mkdirp(plan.binDir)
  // win32 only: whose install does this bin dir already hold? Read once,
  // before the loop, because the breadcrumb entry is written AFTER the copies
  // and reading it per entry would see our own half-written state.
  const existingBreadcrumb =
    platform === 'win32' ? fx.readFile(joinUnder(plan.binDir, WRAPPER_BREADCRUMB_FILE)) : null
  const recordedRoot = String(existingBreadcrumb ?? '').split('\n')[0].trim()
  const foreignRoot =
    recordedRoot && normalizeShimPath(recordedRoot) !== normalizeShimPath(String(pluginRoot ?? '').trim())
      ? recordedRoot
      : ''
  let ok = true
  for (const entry of plan.entries) {
    if (entry.kind === 'breadcrumb') {
      // win32 has no symlink to inspect: the shims are COPIES, so the
      // breadcrumb naming the plugin root is the only record of whose install
      // owns this bin dir. A breadcrumb naming a DIFFERENT root is another
      // install's, and the same rule applies as on posix.
      if (!force && foreignRoot) {
        kept.push({ path: entry.to, pointsAt: foreignRoot, wanted: entry.from, reason: 'foreign-breadcrumb' })
        continue
      }
    ...
    const existing = inspectExistingShim(entry.to, entry.from, fx)
    if (!force && (existing.state === 'foreign-link' || existing.state === 'foreign-file')) {
      // ASK OR ANNOUNCE, NEVER SILENTLY REPLACE (KC, 2026-09-21). Something
      // else already answers to `hoai` here: another checkout, a marketplace
      // copy, or a file the owner wrote. Re-pointing it would report success
      // and change which install the owner's existing command runs, which is
      // exactly the shape of failure this release is about.
      kept.push({ path: entry.to, pointsAt: existing.pointsAt, wanted: entry.from, reason: existing.state })
      continue
    }
  ...
  return { ok, binDir: plan.binDir, wrote, notes, onPath, profiles, kept }
}
