...
import { STEP_KINDS, parseSemver } from './update-planner.mjs'
...
async function rollbackCore(ctx, step) {
  ...
  try {
    ...
    if (snapshot.installPath) {
      ...
      if (pluginJson === null) return impossible('install_path_missing')
...
async function refreshMarketplaceStep(ctx, step) {
  ...
  if (ctx.installMethod === 'marketplace' && ctx.targetVersion === null) {
  ...
  if (refreshed) {
    await finish(ctx, step, 'ok', 'updated', `latest:${ctx.targetVersion ?? 'unknown'}`)
    return
  }
 ...
 */
export async function executePlan(plan, deps, report) {
  ...
  for (const raw of steps) {
    const step = raw && typeof raw === 'object' ? raw : { id: 'invalid', kind: 'invalid' }
    if (!step.id) step.id = `s${String(ctx.steps.length + 1).padStart(2, '0')}-${step.kind}`
    if (ctx.aborted) {
