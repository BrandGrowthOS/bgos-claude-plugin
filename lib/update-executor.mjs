...
import { ensureMarketplaceAutoUpdate } from './claude-preseed.mjs'
 ...
 * @property {(params: {configDir: string, marketplace: string}) => {changed: boolean, reason: string}} [ensureAutoUpdate]
...
function makeContext(plan, deps, report) {
  ...
  // Injectable, and deliberately NOT routed through ctx.fs: ctx.fs is async and this helper is sync,
  // so one of them would have to change to wire them together. The seam is also what keeps the test
  // suite off the real ~/.claude, which a default calling straight into node:fs would not.
  const ensureAutoUpdate =
    typeof deps.ensureAutoUpdate === 'function' ? deps.ensureAutoUpdate : ensureMarketplaceAutoUpdate
...
async function rollbackCore(ctx, step) {
  ...
  try {
    ...
    if (snapshot.installPath) {
      const jsonPath = joinLike(snapshot.installPath, '.claude-plugin', 'plugin.json')
      // Probed separately from the read, because the two states need different advice and the read
      // alone cannot tell them apart: a null comes back both when the directory is gone and when it
      // is there but unreadable. Never throws, so a failing probe reads as absent, which is the
      // conservative half.
      let present = false
      try {
        present = Boolean(await ctx.fs.exists(jsonPath))
      } catch {
        present = false
      }
      const pluginJson = await readText(ctx, jsonPath)
      if (pluginJson === null) {
        ...
        return impossible(present ? 'install_path_unreadable' : 'install_path_missing')
      }
...
async function registerMarketplaceStep(ctx, step) {
  ...
  if (out.kind === 'registered' || out.kind === 'already') {
    // AFTER the add, never before. `claude plugin marketplace add` rewrites the marketplace's entry
    // in settings.json down to just its source, on BOTH branches: a fresh registration and an
    // already-registered one. Verified live against claude 2.1.251 in an isolated config dir, with a
    // sibling marketplace's own autoUpdate left untouched, so this is the add rewriting OUR entry
    // rather than the CLI normalising the whole block. Any enrolment written earlier is therefore
    // silently undone here, which is precisely how a machine stays on one version forever while
    // every step of every run reports success.
    //
    // The other two `add` call sites already re-enrol. This was the third and it did not.
    let detail = out.detail
    try {
      const enrolled = ctx.ensureAutoUpdate({
        configDir: ctx.configDir,
        marketplace: HOAI_MARKETPLACE,
      })
      detail = `${detail};auto_update:${enrolled.reason}`
    } catch (err) {
      // A settings write must never fail an update step. Say what happened in the ledger instead,
      // because a machine that quietly stopped self-updating is the failure we are here to prevent.
      detail = `${detail};auto_update_failed:${errorMessage(err)}`
    }
    await finish(ctx, step, 'ok', out.kind, detail)
    return
  }
