/**
 * The floor's two small files on the host (0.53.0): the ATTACHED marker and
...
/**
 * The evidence kept in a record: the WHOLE matched simple command, up to this.
 *
 * It used to be 3500, cut from the head, and that was a hole: the part that
 * makes a command listed is often at its END (the redirect of
 * `echo "K=<4000 characters>" > .env`, the `--force` after three hundred
 * branch names), so the record kept a head the list no longer read, the
 * server answered `hold: false` and, with the owner's switch ON, the relay
 * auto approved it. The record is a local file the relay deletes when it takes
 * it, so it keeps the command whole, and the RELAY fits it to each limit by
 * dropping arguments, never the operator (lib/floor-evidence.mjs). The cut is
 * not done here because the hook has a 1.5 second budget and a hook that
 * overruns is a floor that is off. A command over this is still cut from its
 * head, and the relay then routes a body the list cannot read to the owner
 * instead of auto approving it (lib/floor-check.ts).
 */
export const FLOOR_RECORD_EVIDENCE_MAX = 64 * 1024
...
export function markFloorAttached({ root, folders, pid = process.pid, now = Date.now(), fs = defaultFs }) {
  ...
  for (const folder of folders) {
    ...
    try {
      ...
      /* a marker that cannot be written leaves the hook silent, as before 0.53.0 */
    }
  }
  return keys
}
