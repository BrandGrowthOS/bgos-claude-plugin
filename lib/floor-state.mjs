/**
 * The floor's two small files on the host (0.46.0): the ATTACHED marker and
 ...
 * (CLAUDE_PROJECT_DIR, the payload's cwd, or a folder above that cwd) whose
 * pid is still alive. A crashed daemon leaves a marker with a dead pid, which
...
export function sessionFolders(payload, env = process.env) {
  const folders = []
  const project = env && typeof env.CLAUDE_PROJECT_DIR === 'string' ? env.CLAUDE_PROJECT_DIR : ''
  if (project.trim()) folders.push(project)
...
export function markFloorAttached({ root, folders, pid = process.pid, now = Date.now(), fs = defaultFs }) {
  ...
  for (const folder of folders) {
    ...
    try {
      ...
      /* a marker that cannot be written leaves the hook silent, as before 0.46.0 */
    }
  }
  return keys
}
