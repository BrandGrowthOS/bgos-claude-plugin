...
export function recipeLaunchCommand({
  ...
  if (platform === 'win32') {
    if ([cwd, nodePath, core].some((v) => String(v).includes('"'))) return null
    return {
      how: 'recipe-console',
      file: comspec,
  ...
  if (platform === 'darwin') {
    ...
    return {
      how: 'recipe-terminal',
      file: 'osascript',
      args: ['-e', `tell application "Terminal" to do script "${line.replace(/"/g, '\\"')}"`],
      spawnOpts,
    }
  }
...
export async function restartAgent(agent, deps = {}) {
  ...
  try {
    const spawned = spawnDetached(command.file, command.args, { ...command.spawnOpts, env })
    return {
      ok: true,
      how: command.how,
