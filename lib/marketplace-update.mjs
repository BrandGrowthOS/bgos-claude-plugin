...
export function mapSupervisedKind(kind) {
  ...
  // 'keepalive' deliberately lands here. The planner's supervisors name the
  // mechanism the EXECUTOR can drive (a restart marker, a service unit), and a
  // keepalive has neither: its restart is a SIGTERM the daemon sends itself in
  // its own ladder, after this run returns. Planning 'launcher-live' would
  // promise a marker nothing watches.
  return 'none'
}
