 ...
 *           install = schtasks /Create /F /SC ONLOGON /RL LIMITED, start =
 *           schtasks /Run. PATH independent: the absolute node path recorded
...
function schtasksSpec({ nodePath, bundleDir }) {
  for (const [name, value] of [['nodePath', nodePath], ['bundleDir', bundleDir]]) {
    if (String(value).includes('"')) {
      throw new Error(`watcherServiceSpec: ${name} contains a double quote, which cannot be embedded in the vbs launcher or the task action`)
    }
  }
  ...
  return {
    ...
    bundleDir,
    dirs: [logsDir],
    files: [{ path: vbsPath, content, mode: null }],
    installCommands: [
      command('schtasks', [
        '/Create',
        '/F',
    ...
    startCommands: [command('schtasks', ['/Run', '/TN', WATCHER_TASK_NAME])],
    stopCommands: [command('schtasks', ['/End', '/TN', WATCHER_TASK_NAME], true)],
    uninstallCommands: [
      command('schtasks', ['/End', '/TN', WATCHER_TASK_NAME], true),
      command('schtasks', ['/Delete', '/F', '/TN', WATCHER_TASK_NAME], true),
    ],
    statusCommands: [command('schtasks', ['/Query', '/TN', WATCHER_TASK_NAME, '/FO', 'LIST'], true)],
  }
}
