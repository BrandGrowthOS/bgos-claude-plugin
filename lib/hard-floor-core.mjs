 ...
 * happens to name its server `bgos` is not exempt; the HOAI Agent Browser's
 * exact server names (HOAI_AGENT_BROWSER_SERVERS) are left to the browser's
 * own gate the same way.
 *
...
export const HARD_FLOOR_RULES = Object.freeze([
  ...
  Object.freeze({ id: 'acts_on_owners_behalf', words: 'sending, posting, paying or deleting on your behalf' }),
])
...
/**
 * The HOAI Agent Browser's MCP server, and ONLY it, by exact name, the same
 * list as the server's HOAI_AGENT_BROWSER_SERVERS: the standalone shim
 * (`hoai-browser`) and the plugin installs that ship it (`hoai`, and `hoaiq`
 * for the QA install; the plugin was already `hoai` when the browser shipped
 * in 0.39.2). NOT the channel: its tools act in a browser for the agent, and
 * they are left to the browser's OWN gate, which stops every sensitive action
 * in the page and asks the owner, so a floor on top would be a second copy of
 * that rule. The only browser tools the word list catches are its storage
 * clearers (`browser_cookie_delete`, `browser_localstorage_delete`,
 * `browser_sessionstorage_delete`), which send, post and pay nothing (the
 * stage 6 backend review). Another plugin that names its server
 * `hoai-browser` is NOT exempt.
 */
export const HOAI_AGENT_BROWSER_SERVERS = Object.freeze([
  'hoai-browser',
  'plugin_hoai_hoai-browser',
  'plugin_hoaiq_hoai-browser',
])
...
/**
 * Paying, in the names payment tools really use, as the server reads it: a
 * payment NOUN only beside a verb that makes one happen (a read of one uses
 * the same noun: `list_payment_intents`, `get_charge`), and `charge` as the
 * tool's first word is the verb itself (`charge_card`). Walked round:
 * `checkout`, `order` and `invoice` names.
 */
const PAYMENT_NOUNS = new Set(['payment', 'payments', 'charge', 'charges'])
...
/** @param {string[]} words @returns {boolean} */
function paysByName(words) {
  if (words[0] === 'charge') return true
  return words.some((word) => PAYMENT_NOUNS.has(word)) && words.some((word) => PAYMENT_VERBS.has(word))
}
...
/** The HOAI Agent Browser's MCP server, by exact name only (HOAI_AGENT_BROWSER_SERVERS). */
export function isAgentBrowserServer(server) {
  return HOAI_AGENT_BROWSER_SERVERS.includes(asString(server).toLowerCase())
}
...
export function classifyToolName(toolName) {
  ...
  if (isAgentBrowserServer(parts.server)) return null
  const words = toolNameWords(parts.tool)
  return words.some((word) => ACTION_WORDS.has(word)) || paysByName(words) ? 'acts_on_owners_behalf' : null
}
