 ...
 * @property {'credentials'|'supervised-folder'} discoveredVia how the inventory found this
 *   agent at all: a ~/.bgos-agent credentials file, or the folder config of a job supervising it
 ...
 * @property {InventoryBasis} [inventory] what the agent list was built from, on every non-blocked
 *                                      plan, so a reader never mistakes this checklist for the
 *                                      whole machine
 */

/**
 * @typedef {object} InventoryBasis
 * @property {number} total
 * @property {number} credentials       rows from a ~/.bgos-agent credentials file
 * @property {number} supervisedFolder  rows found only through a supervised job's folder config
 * @property {string} basis             the same thing in one sentence, including the blind spot
 */
...
/** The platforms the planner knows; anything else plans as 'unknown' (service supervisors ignored, noted). */
...
export function compareSemver(left, right) {
  if (!left || !right) throw new TypeError('compareSemver: both operands must be parsed semver (parseSemver returned null)')
...
export function normalizeAgents(agents) {
  ...
  if (!Array.isArray(agents)) {
    if (agents !== undefined) notes.push(`ignored agents that is not an array (${agents === null ? 'null' : typeof agents})`)
  ...
  for (const raw of agents) {
    ...
    seen.add(id)
    const version = validVersion(entry.version)
    if (version === null && typeof entry.version === 'string' && entry.version.trim() !== '') {
      notes.push(`ignored unparsable version ${JSON.stringify(entry.version)} for agent ${id}`)
    }
    kept.push({
      assistantId: id,
      cwd: cwdOf(entry),
      ...
      discoveredVia: entry.discoveredVia === 'supervised-folder' ? 'supervised-folder' : 'credentials',
      version,
    })
  }
...
/**
 * What this plan's agent list is BUILT FROM, stated on every plan.
 *
 * A machine has no single registry of agents: `bgos-pair` writes
 * ~/.bgos-agent/credentials-<id>.json, while `bgos-agent install --key --user`
 * and `bgos-claim` put the key in the agent folder's .mcp.json and write no
 * credentials file at all. The inventory reads both, but the second kind is
 * only findable through the working directory of a LOADED service job, so an
 * agent that has neither a credentials file nor a loaded job naming its folder
 * cannot be seen from here. It also cannot be restarted from here, but a
 * checklist that just omits it looks complete when it is not. This note is
 * what stops that: it says how many rows came from where, and names the blind
 * spot in one clause.
 * @param {{ assistantId: string, discoveredVia?: string }[]} agents
 * @returns {InventoryBasis}
 */
export function inventoryBasis(agents) {
  const list = Array.isArray(agents) ? agents : []
  const supervisedFolder = list.filter((a) => a.discoveredVia === 'supervised-folder').length
  const credentials = list.length - supervisedFolder
  return {
    total: list.length,
    credentials,
    supervisedFolder,
    basis:
      ...
      `${credentials} from ~/.bgos-agent credentials files, ` +
      `${supervisedFolder} from a supervised job's folder config. ` +
...
export function effectiveSupervisor(agent, platform) {
  ...
  if (supervisor === 'service') return platform === 'darwin' || platform === 'linux' ? 'service' : 'none'
  return 'none'
}
...
function manualReason(agent, platform) {
  const entry = asObject(agent)
  const prefix = platform !== 'darwin' && platform !== 'linux' && entry.supervisor === 'service' ? `service supervisor ignored on ${platform}, ` : ''
 ...
 * @returns {string|null}
 */
function restartReason(agent, intent, mutated, target, runningVersion) {
  ...
  // An agent that does not report its own version inherits the machine's
  // running version (the daemon's own, or what the watcher observed), so an
  // installed-but-not-running plugin still drives a restart.
  const reported = agent.version ?? runningVersion ?? null
  const running = parseSemver(reported)
  const wanted = parseSemver(target)
  if (running && wanted && compareSemver(running, wanted) !== 0) return `agent runs ${reported}, installed ${target}`
  return null
}
...
export function planMachine(state) {
  ...
  const platform = PLATFORMS.includes(platformRaw) ? platformRaw : 'unknown'
  ...
  const inventory = inventoryBasis(agents)
  const notes = [
    ...(input.rollbackLatched === true ? ['updates latched, restart only'] : []),
    ...normalized.notes,
    // `notes` is the anomaly channel, so the basis rides on plan.inventory
    // (always) and only becomes a note when the credentials registry ALONE
    // would have produced a shorter checklist than this one.
    ...(inventory.supervisedFolder > 0
      ? [
          `${inventory.supervisedFolder} agent${inventory.supervisedFolder === 1 ? '' : 's'} ` +
            'found only through a supervised job folder, not a credentials file',
        ]
      : []),
  ]
  if (platform === 'unknown') notes.push(`unknown platform ${JSON.stringify(input.platform ?? null)}, service supervisor ignored`)
  ...
  const runningVersion = validVersion(input.runningVersion)
  // A recipe relaunch needs plugin files on disk: a clone always has them, a
  // marketplace install only when it is present or this plan installs it.
  const pluginAvailable = installMethod === 'clone' || asObject(input.installed).present === true || install.mutated
  let restarted = 0
  for (const agent of agents) {
    const reason = restartReason(agent, intent, install.mutated, target, runningVersion)
    if (!reason) continue
    ...
    if (via === 'recipe' && !pluginAvailable) {
      steps.push(step('manual_restart_required', 'continue', `plugin not installed, cannot relaunch from the recipe: ${reason}`, { target: agent.assistantId }))
      continue
    }
    steps.push(step('restart_agent', 'continue', `restart via ${MECHANISM_LABEL[via]}: ${reason}`, { target: agent.assistantId, via }))
    // Only an agent that was RUNNING before the install can prove the update
    // broke it; one that was already down must not roll a good install back.
    const canBlameInstall = install.mutated && agent.running === true
    steps.push(step('verify_agent', canBlameInstall ? 'rollback' : 'continue', canBlameInstall ? 'channel-live marker newer than restart' : 'channel-live marker newer than restart; agent was down before, no rollback', { target: agent.assistantId }))
    restarted += 1
  }
  ...
  return {
    ...
    steps: withIds(steps),
    notes,
    // Every plan states what its agent list was built from, so a reader can
    // never mistake this checklist for the whole machine.
    inventory,
  }
}
