 ...
 *
 * Refused when the folder carries a HOAI folder pin naming a DIFFERENT
 * assistant. The whole reason a working directory identifies an agent is that
 * the agent's identity comes from that folder; a pin saying the folder belongs
 * to someone else is that assumption failing out loud, and a restart aimed
 * there would start the wrong agent. An absent pin stays usable: most agent
 * folders have none, and the pin is only ever used here to VETO.
 */
function usableCwd(cwd, home, assistantId, readFile) {
  ...
  const raw = readFile(joinDir(value, FOLDER_PIN_FILE_NAME))
  const pin = String(raw ?? '').trim()
  if (/^\d+$/.test(pin) && pin !== String(assistantId ?? '')) return ''
  return value
}
