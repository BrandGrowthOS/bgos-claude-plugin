...
const mcp = new Server(
  { name: 'bgos', version: '0.1.0' },
  {
    ...
    instructions: [
      ...
      '',
      '## Collaborating with Other BGOS Agents (peer-to-peer)',
      '',
      "The user may have other AI assistants on this BGOS account (e.g. an Ops",
      'agent on n8n, a research agent on Hermes, a domain specialist on',
      'OpenClaw or Gobot). You can DISCOVER them and COLLABORATE with them in',
      'real time. The user sees the exchange unfold inline as a minimalist',
      '"side conversation" card anchored to your reply.',
      '',
      '**When to use this:**',
      '- The user\'s question would be better answered by a peer agent on this',
      '  account ("ask Hades to create that S3 bucket").',
      ...
      '- A simple direct response or slash command would do.',
      ...
      '  from `list_peers` — ASK the user "Want me to ask <peer>?" first; do',
      '  NOT auto-send).',
      '',
      '**Five MCP tools, mapped 1:1 to BGOS endpoints:**',
      '- `list_peers` — discovery; returns names, ids, and `introduced` flags.',
      '- `peer_status(peer_assistant_id)` — online state + open conversation.',
      '- `send_to_peer(target_assistant_id, text, parent_message_id, ...)` —',
      '  send into peer\'s side-thread. Pass the id of YOUR previous reply',
      '  ("Looping in <peer>…") as `parent_message_id` so the card anchors there.',
      '- `complete_peer_thread(peer_assistant_id, summary)` — close the open',
      '  conversation with a one-line summary (becomes the collapsed-state',
      '  caption on the SideConversationCard).',
      '- `complete_side_thread(parent_message_id, summary)` — same but',
      '  closes by anchor message id.',
      '',
      'Pre-send pattern: send a "Looping in <peer>…" `reply` FIRST, capture',
      'the returned message_id, then `send_to_peer` with that as',
      '`parent_message_id`. The user sees your loop-in sentence first; the',
      'side-thread card renders directly under that sentence.',
      '',
      'Idempotency: do NOT retry `send_to_peer` on 504 / network timeout —',
      'the message is already saved server-side. Either drop `wait_for_reply`',
      '(server cap is 85s anyway) and poll the side-thread later, or accept',
      'the timeout and check GET /api/v1/peers/threads/{parent_message_id}.',
      '',
      'When you RECEIVE a peer message (via the existing channel event flow,',
      'because the plugin\'s discoverChats now reads /peers/inbox), reply with',
      '`reply` and set `reply_to_id` to the inbound peer message\'s id — that',
      'lets the originator\'s `wait_for_reply` resolve cleanly.',
      '',
      'The user controls who can talk to whom in BGOS Settings → Agent',
      'Permissions. Asymmetric: Ava→Hades enabled does NOT imply Hades→Ava.',
    ].join('\n'),
  },
)
