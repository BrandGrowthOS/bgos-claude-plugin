...
What lets the BGOS app supervise a running session:

- **Stop button (`stop_turn`, cooperative)**: when the user presses Stop for a chat, the backend sends a `stop_turn` RPC over the same WebSocket lane as voice. The plugin cannot kill an in-flight Claude Code turn (there is no process-level cancel hook), so the stop is **cooperative and honest about it**: the plugin pushes a channel notification telling the live agent to stand down on that ONE chat immediately (no new tool calls, one short acknowledgement reply, partial results kept, and the chat's open mission left open rather than completed because of the stop), posts a plain "Asked to stop." confirmation into the chat (it claims the asking and nothing more, because the model has not stood down yet when it is posted), and reports `{stopped: true, mode: 'cooperative'}`. If the live session is unreachable (or the frame has no chat id) it reports `{stopped: false, supported: false}` instead of pretending. Nothing is killed and other chats are never touched.
...
- **A Stop while Keep working is armed (v0.55.0+)**: an armed `/goal` would re prompt the model the moment it stands down, so a cooperative stop would not stick. Where this daemon can type into its session (it declares `mission_pause`) and its goal lane holds an armed goal for the stopped chat's open mission, the Stop also pauses that mission with the reason "Stopped by you" (the native goal is cleared and its condition kept), and the owner's next message in that chat resumes it. A Pause the owner pressed in the app is never resumed by a message. Declared as `stop_pauses_mission`. With no armed goal, or on a host that cannot type (Windows), nothing is written and the notice alone keeps the mission open.
- **Sessions library (`list_sessions`, v0.55.0+)**: the app's Sessions sheet lists the sessions in this agent's own folder (the Claude Code project folder of its working directory, the same one the context gauge reads): a title (the session's own name, else Claude Code's, else the owner's first message with the channel envelope off), when it was last active, a one line preview, the git branch, and a Current tag on the live session. At most the 200 newest transcripts are read, and only the first and last 64 KB of each; an unchanged file is not read again. A title or preview that looks like it holds a secret is withheld whole on this machine before anything leaves it, and search runs only on what is shown. Resuming or renaming a session from the app is a later update: `resume_session` and `rename_session` answer `unsupported`, and the list says so up front. Declared as `sessions_library` on every host.
