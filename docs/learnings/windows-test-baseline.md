...
**Addendum, 2026-09-20 (a CRLF worktree adds seven more).** On a checkout made with `core.autocrlf=true` the node family fails 20, not 13. The seven extra are the same three classes and none of them is a branch: `is well formed: LF endings, no em or en dashes, unique names` and `source uses LF endings and no em or en dashes` (both `test/update-planner.test.ts`, reported as `fixture file` and `module purity`) and `win32 gate helper source: only ever presses Enter after the gate marker is on screen, never blindly` are the CRLF working copy; `bash parses the script cleanly (bash -n)` twice (`test/bgos-agent.static.test.ts`, `test/bootstrap-sh.static.test.ts`) is WSL bash being handed a Windows path by Windows node; `the daemon and the watcher resolve the same authority and the same handle` is launchd, which this box does not have; and `a folder pin makes the same host safe, and silent` joins the credentials block above. Normalising a single file to LF is enough to clear its own scan test, which is how you tell a CRLF artefact from a real failure.
